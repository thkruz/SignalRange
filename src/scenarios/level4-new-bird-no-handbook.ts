...
export const level4NewBirdNoHandbook: ScenarioData = {
  ...
  settings: {
    ...
    groundStations: [
      {
        ...
        rfFrontEnds: [{
          ...
          filter: {
            ...
            isPowered: true,
            selectedFilter: 0, // Student must select
          },
          lnb: {
            isPowered: false,
    ...
    referenceDocuments: [
      {
        id: 'rf-calculations-guide',
        title: 'RF to IF Conversion Reference',
        content: `
          <h3>RF to IF Downconversion</h3>
          <p>The LNB (Low Noise Block) downconverts incoming RF signals to a lower Intermediate Frequency (IF) for processing.</p>
          
          <h4>Basic Formula:</h4>
          <code>IF = RF_input - LO</code>
          <p>Where:</p>
          <ul>
            <li><strong>IF</strong>: Intermediate Frequency (target output)</li>
            <li><strong>RF_input</strong>: Radio Frequency from satellite</li>
            <li><strong>LO</strong>: Local Oscillator frequency</li>
          </ul>
          
          <h4>To Calculate Required LO:</h4>
          <code>LO = RF_input - IF_target</code>
          
          <h4>Example:</h4>
          <p>Given: RF = 3,947.8 MHz, Target IF = 1,247.5 MHz</p>
          <code>LO = 3,947.8 MHz - 1,247.5 MHz = 2,700.3 MHz</code>
          
          <h4>Filter Selection for CW Beacon:</h4>
          <ul>
            <li>CW (Continuous Wave) signals are very narrow - typically 1 kHz or less</li>
            <li>Use narrowest available IF filter (typically 10 kHz for beacon acquisition)</li>
            <li>Wider filters add unnecessary noise</li>
          </ul>
          
          <h4>Spectrum Analyzer Configuration for CW:</h4>
          <ul>
            <li><strong>Center Frequency</strong>: Target IF frequency (1,247.5 MHz)</li>
            <li><strong>Span</strong>: 5-10 kHz (narrow for CW)</li>
            <li><strong>RBW</strong>: 100 Hz (very narrow for CW beacon)</li>
            <li><strong>Reference Level</strong>: Expected signal level + 5-10 dB margin</li>
          </ul>
        `,
      },
      {
        id: 'tidemark-2-ops-note',
        title: 'TIDEMARK-2 Operations Note',
        content: `
          <h3>TIDEMARK-2 First Light Parameters</h3>
          <p><strong>Satellite Position:</strong> 45°W Geostationary</p>
          <p><strong>Spacecraft Status:</strong> Station-keeping achieved, payload active</p>
          
          <h4>Provided Information:</h4>
          <ul>
            <li><strong>Beacon Frequency:</strong> 3,947.8 MHz (C-band downlink)</li>
            <li><strong>Beacon Type:</strong> CW (Continuous Wave)</li>
            <li><strong>Expected Signal Level:</strong> -93 dBm ± 2 dB</li>
            <li><strong>Polarization:</strong> Horizontal</li>
          </ul>
          
          <h4>Ground Station Configuration:</h4>
          <ul>
            <li><strong>Standard IF Target:</strong> 1,247.5 MHz</li>
            <li><strong>LNB Gain:</strong> 55 dB (standard)</li>
            <li><strong>Antenna Pointing:</strong> Az 219.7°, El 26.3° (from VT-01)</li>
          </ul>
          
          <h4>Notes:</h4>
          <p>This is a newly operational satellite. First light acquisition is critical for
          SeaLink to begin commissioning the communications payload. Accurate frequency
          calculations are essential for successful beacon acquisition.</p>
        `,
      },
    ],
  },
