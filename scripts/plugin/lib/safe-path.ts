 ...
 * `baseDir`.
 *
 * These scripts are run from the CLI (and by agentic tooling), so any path
 * derived from `process.argv` must be confined to a known-safe root before it
 * reaches the filesystem; otherwise a faulty argument like `../../..` or an
 * absolute system path could read, create, or delete files outside the
