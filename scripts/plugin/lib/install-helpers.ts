...
import { existingIds } from './existing-ids';
...
/**
 * Strict validation used by `add`/`update`/`create`: throws CliError on the
 * first schema, disk-consistency, or id-collision error. Prints non-fatal
 * warnings.
 */
...
/** True when `absPath` is a strict descendant of src/plugins-external. */
