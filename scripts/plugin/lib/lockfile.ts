...
export function readLockfile(): Lockfile {
  ...
  const parsed = JSON.parse(readFileSync(LOCKFILE_PATH, 'utf8')) as { formatVersion?: number; plugins?: Record<string, LockEntry> };

  return { formatVersion: parsed.formatVersion ?? FORMAT_VERSION, plugins: parsed.plugins ?? {} };
}
