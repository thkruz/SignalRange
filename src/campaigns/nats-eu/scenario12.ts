        ...
        },
        {
          ...
          params: {
            ...
            options: [
              ...
              'Because the payload rejects PLD-TEST-PATTERN unless PLD-ON was the previous command.',
            ],
            correctIndex: 0,
            explanation:
              'A command ACK proves the spacecraft received the command, not that the command did anything. The decode is what proves PLD-ON worked, and the card is only as good as the order it was filled in. Eighteen minutes to AOS.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        ...
        },
        {
          ...
          params: {
            ...
            options: [
              'PLD-ON and the pattern command go up under the command key; the test pattern comes down under the traffic key - both have to be Valid before either half of the test means anything',
              'Only the command key; the test pattern is unencrypted on a new payload',
              'Only the traffic key; the TT&C console has its own key that is always valid',
              'Neither: acceptance is done in crypto bypass so the numbers are clean',
            ],
            correctIndex: 0,
            explanation:
              'Two keys, two directions. A pattern that will not decrypt looks exactly like a payload that did not power on, and the card cannot tell them apart afterwards. Record both Valid before AOS so the decode can only mean one thing.',
            pointPenalty: 5,
      ...
      conditions: [
        ...
        },
        {
          ...
          params: {
            ...
            options: ['1465 MHz (RF minus LO - low-side LO adds on the way up)', '1405 MHz (that is SAR-1)', '1315 MHz (the beacon IF)', '26665 MHz (LO plus RF)'],
            correctIndex: 0,
            explanation:
              'Low-side on the uplink: RF = IF + LO, so IF = 14065 - 12600 = 1465. The receive side is high-side (IF = LO - RF); the two sides of this rack do their arithmetic with opposite signs, and the same 60 MHz mistake is possible on both.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        ...
        },
        {
          ...
          params: {
            ...
            options: [
              'The CW beacon at 1315, the 36 MHz test pattern from 1322 to 1358, and the 1290 MHz transponded return of your own uplink just off the left edge',
              'The beacon at 1315 and SAR-1 imagery at 1414',
              'Only the test pattern; the beacon is outside the video band',
              'The TT&C uplink at 1465, mirrored by the high-side LO',
            ],
            correctIndex: 0,
            explanation:
              'Know the neighbours. That 1290 MHz return is the station hearing itself through the bird, and it is about to matter more than anything else on this screen.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        ...
        },
        {
          ...
          params: {
            ...
            options: [
              'Every stage sees a signal before it sees gain: an HPA enabled with no drive amplifies the noise floor into the feed, and a BUC unmuted with the modem off does the same at lower level',
              'Because the BUC cannot unmute while the HPA is enabled',
              'It is convention; either order works on a Ku rack',
              'So the command key can be validated before the carrier radiates',
            ],
            correctIndex: 0,
            explanation: 'HPA_NOISE_AMPLIFICATION is the invariant this order protects. On the way down it is the mirror image: HPA off first, then mute, then the carrier.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        ...
        },
        {
          ...
          params: {
            ...
            options: [
              "SAR-3's command transponder turns the station's own 14065 MHz carrier around at 11810 MHz, co-polar and inside the receive band, some 50 dB above the pattern; the AGC pulls the whole chain down and the video goes with it",
              'The HPA heats the LNB through the feed and raises the noise temperature',
              'Doppler compensation on the uplink is dragging the receiver off frequency',
              'The payload cannot transmit the pattern while it is receiving commands',
            ],
            correctIndex: 0,
            explanation:
              'The station is hearing itself through the bird. Nothing on the RX chain is broken; it is doing exactly what an AGC does with a strong in-band signal. Take the strong signal away and the number comes back.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
        ...
        },
        {
          ...
          params: {
            ...
            options: [
              ...
              'The committed margin, since that is what the modem actually used.',
            ],
            correctIndex: 0,
            explanation:
              'A measurement that matches the model is what lets Rotterdam and the customer predict every pass after this one. A measurement with no prediction beside it is a good day, not an acceptance.',
            pointPenalty: 5,
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              ...
              'ACCEPTED, with the residual risk that the decode was done with the uplink secured rather than live.',
            ],
            correctIndex: 0,
            explanation:
              'Acceptance is a statement about what was tested, no more. Writing down what was not tested is what makes the card usable when the first low pass comes in under budget. Decoding with the uplink secured is the procedure, not a risk.',
            pointPenalty: 5,
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              ...
              'Tasking on Galway passes above 28 degrees only, until Shetland has run its own card.',
            ],
            correctIndex: 0,
            explanation:
