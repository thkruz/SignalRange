...
export interface RestoreResult {
  ...
  readonly localSkipped: string[];
  readonly failed: string[];
}

/**
 * Clone any lockfile entry missing from disk and check out its pinned commit.
 * Network/clone failures warn and continue: a missing plugin is later dropped
 * from the generated manifest by sync, so the build still succeeds without it.
 * Local entries are skipped with a warning.
 */
...
export function restoreCommand(): number {
  ...
  log.step('Run "pnpm run plugin -- sync" to regenerate the manifest (or just build).');
