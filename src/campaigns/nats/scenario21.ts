      ...
      conditions: [
        ...
        },
        {
          ...
          params: {
            ...
            options: [
              'As an interference incident: characterize, discriminate, mitigate, document - and verify the data layer separately',
              'As a transient: watch, wait, time-stamp, log - and act only if it is still there at the end of the shift',
              'As a jamming attack: disconnect TM-2, notify Dana, isolate the network, escalate - and characterize it afterward',
              'As a receiver fault: power-cycle the LNB, re-lock, re-verify, log - and treat intermittent as a flaky front end',
            ],
            correctIndex: 0,
            explanation:
              'Evidence first, conclusion last. Whether it ends as "misaligned partner uplink" or "deliberate denial," the work is identical until the evidence decides - and that discipline is what makes the eventual conclusion defensible. The same evidence resolves an accident or builds the case for deliberate interference.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              'Against accident, toward deliberation: accidents are continuous or random; a clean minute-scale on/off cadence implies a switch',
              'Toward radar, away from intent: rotating antennas sweep past on a fixed period; a minute-scale on/off cadence is a scan rate',
              "Toward a failing supply on a neighbor's uplink: a dying regulator cycles as it heats; a minute-scale cadence is thermal",
              "Toward nothing: duty cycle is not diagnostic; on/off timing depends on the receiver's AGC, not on the source of the noise",
            ],
            correctIndex: 0,
            explanation:
              'Radar duty cycles are seconds, not minutes. Accidental interference is continuous or erratic. A deliberate, minute-scale on/off pattern implies a hand on a switch or an automated transmitter, not weather or a stuck oscillator - one of the strongest behavioral indicators of intent, though it is an indicator, not proof.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              'It is in the UPLINK - the satellite relays it to every station, so the source is transmitting at the bird, not local to Maine',
              'It is LOCAL terrestrial - all three sites sit under the same radar and 5G footprint, so each is hearing its own ground source',
              'It is the TRANSPONDER - a failing payload puts noise on every downlink, so the satellite itself is the source, not an uplink',
              'It is EXPECTED - every station on the bird sees every signal the bird relays, so cross-station visibility tells you nothing',
            ],
            correctIndex: 0,
            explanation:
              'This is the highest-information test in the matrix. Local interference appears at one station; uplink interference is relayed to all. Every-station visibility moves the source from "somewhere near Maine" to "transmitting at the satellite." That rules out radar, 5G, and our own equipment, and points at a misaligned or deliberate uplink.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              "It matches nothing in the database: cross-pol would mirror a neighbor's carrier, an errant uplink would sit at a known slot",
              "It matches cross-pol leakage: a neighbor's carrier on the other polarization bleeds in, sized like ours because it is a peer",
              'It matches a coordinated adjacent-satellite carrier: the database lists a peer at this slot, sized close to ours by design',
              'It matches nothing because shape means nothing: bandwidth and placement vary with the analyzer span, not with the source',
            ],
            correctIndex: 0,
            explanation:
              'Accidents look like known things in the wrong place. Noise deliberately shaped and placed to deny a specific carrier looks like nothing the coordination database can account for - the absence of a benign match is itself evidence. Broadband noise shaped to our carrier inside our passband fits neither benign case.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              'A DENIAL attack on the RF layer - it blocks the signal but never touches data, crypto, or keys; intrusion escalates differently',
              'An INTRUSION on the data layer - a jammer with this much control has system access; every jamming event escalates as a breach',
              'A MALWARE attack on the modem - the on/off cadence is a compromised firmware loop; crypto and keys must be assumed lost',
              'An ATTACK, and the class is academic - denial or intrusion, the response is the same; escalation does not change with the label',
            ],
            correctIndex: 0,
            explanation:
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              'Confirms RF denial only - degraded, not breached; a crypto change or invalid keys mid-incident flips it to a security incident',
              'Confirms nothing - crypto is a data-layer state; interference lives on the RF layer, so no crypto reading could flip the incident',
              'Confirms the interferer is identified - its frames decrypted under our keys; a key mismatch would flip it back to unknown source',
              'Confirms the jamming has stopped - frames pass clean when the noise is off; a return of the noise flips it back to an active incident',
            ],
            correctIndex: 0,
            explanation:
              'Two layers, two verifications, never conflated. Intact crypto bounds this to a denial-of-availability event. The hypothetical - a crypto change mid-incident - is exactly the tripwire that would escalate it to an intrusion, with a different escalation: Dana, then the security officer, and nothing holding keys gets power-cycled.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              'Notch filter on the interferer band - excise its energy for a little SNR in that slice; reversible and customer-preserving',
              'Raise uplink power to "burn through" - out-drive the noise for a few dB of HPA headroom; fast and needs no receiver change',
              'Retune the customer carrier to a clean slot - move away from the noise for a coordination request; permanent and neighbor-safe',
              'Mute the receiver until the interferer stops - drop the noise at the cost of the carrier during each burst; simple and reversible',
            ],
            correctIndex: 0,
            explanation:
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              'It breaks coordination and degrades every neighbor on the transponder - a power war escalates the incident and still loses',
              "It would work but the HPA cannot ramp fast enough - by the time the drive is up, the jammer's ninety-second burst is already over",
              "It breaks the customer contract, which fixes uplink power - a power change needs SeaLink's sign-off before it can be commanded",
              'It is fine in principle, since more power always improves the link - the objection is only that it costs HPA lifetime',
            ],
            correctIndex: 0,
            explanation:
              "Burn-through is a denial response that creates more denial: you cannot out-shout an uplink jammer from the ground, and trying harms bystanders. The operator's tools are receive-side mitigation and the regulator; offense belongs to the spectrum authority and geolocation services, not the victim's HPA.",
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              'Victim and interferer parameters (freq, BW, duty cycle), spectrum captures, cross-station confirmation, impact - all timestamped',
              'Interferer identity and location (operator, site, uplink dish), our ticket number, service impact - all determined here before filing',
              'A one-line summary (transponder, time, "being jammed"), our ticket number, a request for help - all the regulator needs to start',
              'Our internal ticket (number, opened time, owner), the notch settings, the customer name - all filed, details available on request',
            ],
            correctIndex: 0,
            explanation:
              'Per ITU-R practice the report is evidence, not conclusion: victim (satellite, transponder, pol, center frequency, bandwidth), interferer (center frequency, bandwidth, measured duty cycle, first and last seen), captures at documented settings, cross-station confirmation, impact, and mitigation applied - everything timestamped. Attribution (who, where) belongs to the regulator and geolocation providers, not the ground operator.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              '"Signature consistent with deliberate uplink interference" - confidence-bounded; attribution is for the regulator, not us',
              '"Confirmed hostile jamming by [named actor]" - state it plainly; the evidence is strong enough to name the source ourselves',
              '"Probably just interference, no action needed" - keep it low-key; the regulator can characterize it if it ever returns',
              '"Interference event, raw data attached" - no characterization; the regulator draws every conclusion from the captures',
            ],
            correctIndex: 0,
            explanation:
              'The evidence supports "consistent with deliberate" - it does not support naming an actor or declaring intent as fact. Intent, like attribution, is for the regulator to determine. Disciplined language is what makes your evidence usable; overreach is what gets it dismissed.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
      ...
      conditions: [
        {
          ...
          params: {
            ...
            options: [
              'TM-2 TP-1 interference: noise ~1470 MHz IF, 90s/60s duty, uplink-confirmed (VT-01 + Halifax); crypto intact - RF denial, notched',
              'TM-2 TP-1 interference: noise ~1470 MHz IF, 90s/60s duty, local source suspected (Maine only); LNB power-cycled - RX issue, cleared',
              'TM-2 TP-1 interference: hostile jamming confirmed, actor named, burned through (HPA raised); link saved - regulator not needed',
              'TM-2 TP-1 interference: some noise near the carrier, intermittent, source unknown (not chased); notch applied - fixed, no follow-up',
            ],
            correctIndex: 0,
            explanation:
              'Everything that matters: signature, duty cycle, the uplink confirmation, the intact data layer (crypto ACTIVE, keys Valid - denial not intrusion), the mitigation with the carrier holding, the disciplined finding (signature consistent with deliberate interference, regulator package filed), and the escalation tripwire (monitoring; escalate on any data-layer change). The next shift inherits a complete, defensible picture.',
            pointPenalty: 5,
          },
          mustMaintain: false,
        },
